...
#### INHA 
```{r}
col <- c("Stage I" = , "Stage II" = ,"Stage III"= , "StageIV"=)
...
```
```{r}
#coloring this by stage 
counts <- counts(dds['ENSG00000049768.14',], normalized = TRUE)
m <- list(counts = as.numeric(counts), group= sample_meta_data$Activin_Level)
m <- as_tibble(m)
q <- ggplot(m, aes(group, counts)) + geom_boxplot(aes(fill= sample_meta_data$figo_stage ))+ aes(color= sample_meta_data$figo_stage) + theme(text = element_text(size = 9)) + theme(axis.text.y = element_text(size = 10)) + scale_x_discrete(limits=c("Low","Intermediate","High"))
q <- q + labs(x = "Activin Levels", y = "Normalized Counts ", title = "Expression of FOXP3")
q
```
```{r}
ggsave("~/Desktop/Elizabeth_Figures/activin_FOXP3_by_stage.tiff", plot = last_plot())
write.csv(counts, "~/Desktop/Elizabeth_Figures/counts_activin_FOXP3_by_stage.csv")
```
